...
class PlottingManager():
    ...
    def plotData(self,filename, xcol, ycol, xaxis, yaxis, titleIn, labelIn):
        # plt.clf()
        [ty,tx] = self.getData(filename,xcol,ycol,self.plotLogX,self.plotLogY) #used to be [tx,ty]
        if self.fitData:
...
class MainWindow(wx.Frame):
    def __init__(self,parent,id,title):
        wx.Frame.__init__(self,parent, wx.ID_ANY, title, size = (900,900), style=wx.DEFAULT_FRAME_STYLE|wx.NO_FULL_REPAINT_ON_RESIZE)
        #size = (750,600)
